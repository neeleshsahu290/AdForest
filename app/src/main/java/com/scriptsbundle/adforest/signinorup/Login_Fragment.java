...
//        linkedinLoginButton.setText(linedinText);
        ...
        textViewWelcome.setGravity(Gravity.CENTER_HORIZONTAL);
        if(settingsMain.getRTL()){
            password.setGravity(Gravity.RIGHT);
            emailid.setGravity(Gravity.RIGHT);
            forgotPassword.setGravity(Gravity.LEFT);
        }else {
            password.setGravity(Gravity.LEFT);
            password.setGravity(Gravity.LEFT);
            forgotPassword.setGravity(Gravity.RIGHT);
        }
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> responseObj) {
                try {
                    if (responseObj.isSuccessful()) {
                        Log.d("infoLoginresponce", "" + responseObj.toString());

                        JSONObject response = new JSONObject(responseObj.body().string());
                        if (response.getBoolean("success")) {
                            nestedScroll.setVisibility(View.VISIBLE);
                            Log.d("infoLoginobject", "" + response.getJSONObject("data"));
