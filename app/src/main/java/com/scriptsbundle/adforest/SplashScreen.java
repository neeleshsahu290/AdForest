                ...
                public void onResponse(Call<ResponseBody> call, Response<ResponseBody> responseObj) {
                    try {
                        if (responseObj.isSuccessful()) {
                            Log.d("info_settings_Responce", "" + responseObj.toString());

                            JSONObject response = new JSONObject(responseObj.body().string());
                            if (response.getBoolean("success")) {
                                jsonObjectSetting = response.getJSONObject("data");
                                String veryLongString =jsonObjectSetting.toString();
                                int maxLogSize = 1000;
                                for (int i = 0; i <= veryLongString.length() / maxLogSize; i++) {
                                    int start = i * maxLogSize;
                                    int end = (i + 1) * maxLogSize;
                                    end = end > veryLongString.length() ? veryLongString.length() : end;
                                    Log.d("info_settings_Responce", veryLongString.substring(start, end));
                                }

                                setting.setMainColor(jsonObjectSetting.getString("main_color"));

                                isRTL = jsonObjectSetting.getBoolean("is_rtl");
                                Log.d("is_rtl", String.valueOf(isRTL));
                              ...
                              /*  app_show_languages = jsonObjectSetting.getBoolean("app_show_languages");
                                ...
                                try {
                                    setting.setypeofSearch(extraText.getString("type_to_search"));
                                    setting.setokic(extraText.getString("okic"));
                                    setting.setclear_all(extraText.getString("clear_all"));
                                }catch (Exception e){}
                                            ...
                                            if (getIntent().hasExtra("payload")) {
                                                ...
                                                try {
                                                    for (String key : getIntent().getExtras().keySet()) {
                                                        ...
                                                        Intent in = new Intent(SplashScreen.this, ChatActivity.class);
                                                        in.putExtra("adId", getIntent().getExtras().get("adId").toString());
                                                        in.putExtra("recieverId", getIntent().getExtras().get("senderId").toString());
                                                        in.putExtra("calledFromSplash", true);
                                                        in.putExtra("senderId", getIntent().getExtras().get("recieverId").toString());
                                                        try {
                                                            Log.d("namepar",getIntent().getExtras().get("senderName").toString());
                                                            in .putExtra("name",getIntent().getExtras().get("senderName").toString());
                                                        }catch (Exception e){
                                                            Log.d("namepar",e.toString());
